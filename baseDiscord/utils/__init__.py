...
def new_logger(name: str, *,
               ...
               file: str = None,
               frequency: str = None,
               backupcount: int = 7) -> __logging.Logger:
    """Create a new logger
    ...
    OPTIONAL[file] : str
        The path of file if you want save logs in the file. If `frequency` is used, \
        file is a model for files path.
        Default: None
    ...
    if file and not frequency:
        file_handler = __logging.FileHandler(file,
                                             mode='a',
                                             encoding=None,
                                             delay=False)
    elif file and frequency:
        file_handler = logging.BaseDiscordTimedRotatingFileHandler(
            file,
            mode='a',
            extension="log",
            datefmt="%Y%m%d",
